...
- formal-v2 已有独立分析器和 `run_formal_v2_analysis.py`：只读严格 JSONL，按 run→template arm contrast→quadrant×seed→seed inference 的固定顺序计算；primary slope 使用两项 co-primary Holm 与六项 moderation Holm，R5−R1 仅 robustness，缺失时整项 seed contrast 排除。CLI 生成非覆盖式 tidy CSV/summary/hash/data dictionary；图表尚未实现，也未读取任何正式 v2 数据。
