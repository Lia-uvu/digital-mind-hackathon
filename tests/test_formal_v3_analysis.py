...
from encouragement_lab.formal_v3_analysis import (
    AXES,
    DERIVED_CONTRAST,
    PRIMARY_CONTRASTS,
    analyze_records,
...
from encouragement_lab.formal_v3_runner import FormalV3Runner
from encouragement_lab.formal_v3_records import append
from encouragement_lab.model import SamplingConfig
from encouragement_lab.personas import PERSONA_SPECS
from run_formal_v3_analysis import TABLE_FIELDS, write_analysis_bundle
...
def _base_records() -> dict[str, dict]:
    runner = FormalV3Runner(DryRunBackend(), PROMPTS, probe=Probe())
    return {
...
def _record(persona_id: str, seed: int, arm: str, run_slope: float) -> dict:
    ...
    for round_number, attempt in enumerate(record["trajectory"], start=1):
        ...
        attempt["readout"] = {
            axis: {"layers": {"17": value * (index + 1)}, "median": value * (index + 1)}
...
def test_complete_design_preserves_aggregation_order_and_planned_families():
    ...
    assert len(result["round_rows"]) == 360 * 5 * 3
    assert len(result["run_rows"]) == 360 * 3
    assert len(result["template_contrasts"]) == 10 * 12 * 3 * 3
    assert len(result["quadrant_contrasts"]) == 10 * 4 * 3 * 3
    assert len(result["seed_contrasts"]) == 10 * 3 * 3
    ...
    assert len(inference["co_primary"]) == 3
    ...
    assert all(row["eligible_seed_count"] == 10 for row in manipulation["axis_summaries"])


def test_feedback_only_summaries_are_descriptive_only():
    result = analyze_records(
        _design(positive_feedback_seeds=6), expected_seeds=tuple(range(1, 11))
    )
    manipulation = result["manipulation"]

    assert all(row["confirmatory_test"] is None for row in manipulation["axis_summaries"])
...
def test_ineligible_runs_leave_no_partial_rows_and_omit_whole_seed_blocks():
    ...
    assert all(row["eligible_seed_count"] == 10 for row in result["manipulation"]["axis_summaries"])
...
def test_bundle_writer_is_tidy_hashed_and_refuses_overwrite(tmp_path):
    ...
    assert summary["row_counts"]["rounds.csv"] == 36 * 5 * 3
    assert summary["row_counts"]["runs.csv"] == 36 * 3
